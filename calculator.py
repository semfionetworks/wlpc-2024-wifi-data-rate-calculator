...
def get_tdft_tgi_subcarrier_data_rate(
    ...
    channel_width_number = int(
        channel_width_selection.replace("MHz", "").replace(" ", ""))
...
if client_device_details != {} and channel_width_selection is not None:
    ...
    coding_rates = get_coding_rates(
        client_device_details, mcs_indexes, configs)
    nss_list = get_nss_list(client_device_details)
    tdft_tgi_subcarrier_data_rate = get_tdft_tgi_subcarrier_data_rate(
        client_device_details, configs, channel_width_selection)
    ...
    st.toast(
        f'New data rates calculated for {client_device_option} for a {channel_width_selection} wide channel.')
    ...
    with stylable_container(
        ...
        with col2:
            list_nss = [nss for nss in range(
                1, client_device_details['nss'] + 1)]
            nss_filter = st.multiselect(
                'Number of Spatial Streams',
                list_nss,
                default=list_nss
            )
            gi_list = [0.8, 1.2, 3.2] if client_device_details['phy'] in [
                'HE', 'EHT'] else [0.4, 0.8]
